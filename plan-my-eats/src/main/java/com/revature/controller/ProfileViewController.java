...
// I will do this tomorrow morning 
	// here you will have to show all user preferences bmi recipes / restaurants/ etc 
	
	// okay so we get the user by username and then show all their info on the profile page 
	...
	public ResponseEntity<User> getUserInfo(@RequestBody User user) {
		...
		if(u == null || u.getPreferences().isEmpty()||u.getHeight()==0||u.getWeight()==0||u.getAge()==0) {
			//User has an account but so info is available for some reason
			return new ResponseEntity<User>(HttpStatus.NO_CONTENT); 
		} else  {
			return new ResponseEntity<User>(u, HttpStatus.OK);
		}
